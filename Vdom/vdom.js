...
function createElement(tag, data = null, children = null) {
  ...
  if (typeof tag === 'string') {
    ...
    flags = VNodeType.COMPONENT // 组件 未完待续
  } else {
  ...
  if (Array.isArray(children)) {
    ...
    if (length === 0) {
      childFlags = ChildTyps.EMPTY // 没有 children
    } else {
      childFlags = ChildTyps.MULTIPLE // 多个子节点，且子节点使用key
    }
  } else if (children == null) { // 没有子节点
    childFlags = ChildTyps.EMPTY
  } else { // 其他情况都作为文本节点处理，即单个子节点，会调用 createTextVNode 创建纯文本类型的 VNode
...
function createTextVNode(text) {
  return {
    flags: VNodeType.TEXT, // flags 是 VNodeType.TEXT
    tag: null,
    data: null,
    children: text, // 纯文本类型的 VNode，其 children 属性存储的是与之相符的文本内容
    childFlags: ChildTyps.EMPTY // 文本节点没有子节点
  }
}
// 更新data
...
function mount(vnode, container, refNode) {
  const { flags } = vnode  // 体现 flags 的作用
  if (flags == VNodeType.HTML) { // 挂载普通标签
    mountElement(vnode, container, refNode)
  } else if (flags == VNodeType.TEXT) { // 挂载纯文本
